  ...
  >
    ...
    >
      ...
      <div class="flex flex-row justify-between w-full gap-2">
        <div
          class="flex items-center w-8 h-4 duration-300 ease-in-out bg-gray-800 rounded-full hover:cursor-pointer"
          :class="{ 'bg-gray-600 ': !widget.visible }"
          @click="toggleVisibility(widget)"
        >
          <div
            class="w-4 h-4 duration-300 ease-in-out transform bg-gray-500 rounded-full shadow-md"
            :class="{ 'translate-x-4 bg-green-700': widget.visible }"
          ></div>
        </div>
        <div
          v-if="widget.visible"
          @click="toggleLocked(widget, widget.title)"
